...
함수 : 코드를 모듈화 한것으로, 재사용성, 유지보수 효율성이 좋아지는특징을 가짐 
...
// 
    ...
    1. 데이터가 유효한 데이터인지 검사하고 저장하는 위치에 공간 확보가 되었는지 확인하고 파일 저장
       하기 위해 
    2. 파일이 생성되었는지 확인하고
    3. 생성되었다면 파일에 저장한다.
    
    b input태그로부터 데이터 입력받는 경우
            1함수 2함수 3함수 실행

함수의 구성 : 선언부 및 정의부, 실행부
선언부 및 정의부
→ 함수모듈의 출력형태와 이름, 삽입되는 사항과 코드블럭이 추가되어 있는 부분
function 함수이름→func1(obj1,obj2→매개변수){
    res = obj1+obj2;→실행할 부분
    return res;
}
addEventListner("click",이름없는함수→function(){
...
함수 실행순서
브라우저실행 → HTML엘리먼트 호출 → 코드해석 → 함수발견 → 함수실행 
→ 함수실행 완료 후 호출한 위치로 복귀 → 원래코드실행

변수의 존속성
변수의 지역성 : 변수는 기복적으로 코드블럭 내에서만 존재 (코드블럭 - {})
변수 선언 후 코드블럭이 종료될때까지 메모리상에 존재(지역변수)
코드블럭 내에서 선언된 변수는 모두 지역변수이다.
함수 실행시 현재까지 실행되던 내역은 모두 스택이라는 메모리영역에 보관 후 실행함수로 제어를
이동하여 초기화 → 함수실행이 완료되면 초기화된 변수들은 메모리 영역에서 삭제

지역변수는 코드블럭에서만 사용가능
    코드블럭 실행시 생성되고 코드블럭 종료시 삭제

변수의 전역성 : 코드블럭 외부에서 선언된 변수 모든 코드블럭에서 사용 가능
전역변수는 프로그램 종료시 삭제
...
sum(10,20,30,40); → 10과 20 데이터를 매개변수 a와 b에 전달
a = 10, b = 20 저장
